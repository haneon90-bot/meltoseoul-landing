...
const CHARACTERS = {
  ...
  unaffiliated: [
    { id: 'yomiya-shigure', name: '요미야 시구레', aliases: ['청부업자', '무소속'], threat: 'disaster', stigma: '없음 (이레귤러)', ability: '성흔 없이도 일반인 기준을 한참 벗어난 피지컬. 주변 잡동사니가 그대로 흉기가 됨.', bloom: '없음', desc: '소속 불명의 청부업자. 성흔 없음 표시가 오히려 더 수상한 케이스.', height: '190cm', nationality: '일본', quote: '귀찮네. 한 번에 끝내준다.' }
  ]
};
...
function initHanidaeSim() {
  ...
  const script = {
    sigilIntro: { bg: scenes.classroom, video: '/vn-ha-woonjin-intro.mp4', loc: '신입 안내 / 성흔 기초', face: ha.normal, speaker: '하운진', text: '어이, 네가 이번에 들어온 신입이지? 안내 길게 할 시간 없으니까 이것만 들어. 하운진이 자기 어깨를 엄지로 툭 가리켰다. "이게 성흔이야. 이능력을 쓰게 만드는 표식. 관리 못 하면 네 몸부터 망가져."', next: 'start', entryEffect: 'signal' },
  ...
  function render() {
    ...
    root.dataset.media = scene.video ? 'video' : 'image';
    bg.src = scene.bg;
    if(scene.video) {
